...
> 历史设计稿；以下 TODO、状态描述和命令可能已过时。当前入口见 [README](../README.md)。
...
| World model         | Wan2.2 (`RLinf/diffsynth-studio`)                    | **DreamDojo** Cosmos-Predict2.5, LoRA rank-64 lr3e-4 iter-18000 EMA         |
| Reward model        | ResNet success classifier, `resnet_rm.pth`           | **v2 3-head cumulative milestone classifier + ordered ratchet**             |
...
* **GR00T policy config** — `~/Isaac-GR00T` (branch `yunl/pick-trocar`),
  ...
  * action: same 4 keys. GR00T executes 12 policy commands; the online bridge expands them to a 25-sample 30 Hz window, reproduces DreamDojo's 2× subsample/block-delta transform, and emits 12 WM actions.
...
* **Available LeRobot v2.1 datasets on the current machine** (`/localhome/local-yunl/data/`):
  * `pick_trocar_teleop_success_train`   — 225 eps, 34021 frames, 30 fps, **has manual phase annotations** in `meta/episodes.jsonl` under `source_annotation.cleaned_phase_frames` (`left_hand_pickup`, `handover_to_right_hand`, `placed_on_plate`) — labels for the three cumulative milestone heads
...
* **Reward model** — `outputs/milestone/v2/best.pt`, with independent cumulative sigmoid heads `picked`, `handed`, and `placed`. The deployed adapter confirms them in order with thresholds `(0.8, 0.8, 0.8)` and per-env ratchets. Each newly confirmed milestone pays `1`; stage ranges from 0 to 3 and only stage 3 is success.
* **DreamDojo** — `/localhome/local-yunl/DreamDojo`, Cosmos-Predict2.5-based and autoregressive at 15 fps. The selected checkpoint is `lora_r32_lr3e-4_r64_18k/checkpoints/iter_000018000/model_ema_bf16.pt` (CL-MSE 860.0, CL-PSNR 19.05 on the 55-episode holdout).
...
Implemented: real `Video2WorldInference`, 12-frame autoregressive chunks,
28D→43D→384D action conditioning, last-known proprio, batched milestone reward,
selective per-env reset, and built-in DreamDojo component offload.
...
* `convert_to_g1_dex3_action_n1d7(action_chunk, chunk_size=16)` — GR00T action-dict → flat 28-D array in `left_arm, right_arm, left_hand, right_hand` order. Although arms are represented relatively during policy training, GR00T `unapply_action` converts them to absolute targets before this converter runs.
...
2. **Add action-utils branch** in `rlinf/envs/action_utils.py::prepare_actions()`:
   * `elif env_type == SupportedEnvType.DREAMDOJOWM and wm_env_type == "trocar": return raw_chunk_actions` (pass-through — do NOT apply the LIBERO gripper `2x-1` hack; these are already decoded absolute targets).
...
4. **Reward model**: use `BatchedMilestoneReward.score_chunk(frames_btchw)`; it returns sparse per-WM-step milestone payouts and preserves independent stage state.
...
5. **Env config YAML** at `examples/embodiment/config/env/dreamdojo_trocar.yaml` — copy structure from `env/wan_libero_spatial.yaml`, set:
   ...
   * `reward_model.type: MilestoneRewardV2`, `reward_model.from_pretrained: /path/to/outputs/milestone/v2/best.pt`
6. **Recipe YAML** at `examples/embodiment/config/dreamdojo_trocar_grpo_gr00t.yaml` — copy from `wan_libero_spatial_grpo_openvlaoft.yaml`, change:
   ...
   * `actor.model.model_type: gr00t_n1d7`, `model_path: /path/to/g1_pick_trocar_head_10k_bs32_lr1e-4`, `num_action_chunks: 12`, `action_dim: 28`, `obs_converter_type: g1_dex3_wm`
...
11. **Syntax check** (was interrupted before machine switch — repeat on the new host):
    ...
    "
    ```
12. **Single-env dry run** — instantiate `DreamDojoEnv(cfg, num_envs=1, ...)`, call `env.reset()`, feed a random `[1, 12, 28]` action into `env.chunk_step(...)`, and confirm shapes.
