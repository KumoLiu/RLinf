...
def lift_trocars_reward(
    ...
    """
    # Update task stage first
    stage = update_task_stage(env, asset_cfg1, asset_cfg2, table_height, lift_threshold)
...
def trocar_placement_reward(
    ...
    print(f' Place reward: {reward[0].item():.2f} | in_zone: {both_in_zone[0].item()}')
    return reward
